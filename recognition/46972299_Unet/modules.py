...
class Improved3DUnet(nn.Module):    
    ...
    def __init__(self, input_channels: int, num_classes: int) -> None:
        ...
        self.decoder4_localise = Localise(self.FILTER_BASE * 8, self.FILTER_BASE * 8, self.LEAKY_RELU_SLOPE)

        # layer 3
        self.decoder3_upsample = Upsample(self.FILTER_BASE * 8, self.FILTER_BASE * 4, self.LEAKY_RELU_SLOPE)
        self.decoder3_localise = Localise(self.FILTER_BASE * 4, self.FILTER_BASE * 4, self.LEAKY_RELU_SLOPE)
        self.decoder3_segment = Segment(self.FILTER_BASE * 4, self.FILTER_BASE * 4)
        ...
        self.decoder2_localise = Localise(self.FILTER_BASE * 2, self.FILTER_BASE * 2, self.LEAKY_RELU_SLOPE)
        self.decoder2_segment = Segment(self.FILTER_BASE * 2, self.FILTER_BASE * 2)
        self.decoder2_sum = Sum(self.FILTER_BASE * 2, self.LEAKY_RELU_SLOPE)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # output
        out = self.convert(decoder1_sum)
        return torch.softmax(out, dim=self.SOFTMAX_DIMENSION)
...
class Upsample(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        out = nn.functional.interpolate(x, self.input_filters, scale_factor=self.SCALE_FACTOR, mode=self.MODE)
...
class Localise(nn.Module):
    def __init__(self, input_filters: int, output_filters: int, slope: float) -> None:
        ...
        self.process = nn.Sequential(
            ...
            nn.InstanceNorm3d(output_filters),
            nn.LeakyReLU(slope),
            nn.Conv3d(output_filters, output_filters, kernel_size=1, stride=1, padding=1, bias=False),
            nn.InstanceNorm3d(output_filters),
            nn.LeakyReLU(slope)
        )
...
class Segment(nn.Module):
    def __init__(self, input_filters: int, output_filters: int) -> None:
        ...
        self.process = nn.Conv3d(input_filters, output_filters, kernel_size=1, stride=1, padding=1, bias=False)
...
class Upscale(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return nn.functional.interpolate(x, self.output_filters, scale_factor=self.SCALE_FACTOR, mode=self.MODE)
