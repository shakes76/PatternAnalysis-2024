...
class UNET(nn.Module):
    ...
    def forward(self, x, t):
        ...
        for i in range(self.num_layers//2, self.num_layers):
            layer = getattr(self, f'Layer{i+1}')
            #print(layer(x, embeddings)[0].shape, residuals[self.num_layers-i-1].shape)
...
def train(batch_size: int=32,
    ...
    torch.save(checkpoint, 'checkpoints/check')
...
def main():
    train(lr=2e-5, num_epochs=1, checkpoint_path='checkpoints/check')
    inference('checkpoints/check')
