...
class Quantise(nn.Module):
    ...
    def forward(self, inp):
        ...
        if self.training:
            # Calculate the exponential moving average and apply a constant to avoid reaching a saddle point
        ...
        reconstrLoss = F.mse_loss(inp.detach(), quantised)
        ...
        codebookLoss = F.mse_loss(inp, quantised.detach())
        ...
        # perplexity is defined as the amount of times embeddings were accessed, not necessarily the level of entropy when referring to VQVAEs
        perplexity = torch.exp(-torch.sum(avg * torch.log(avg + 1e-10)))
        return quantised, commitmentLoss, codebookLoss, perplexity
