...
class Discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        # Used to embed the AD and CN labels into the input image
        self.label_embedding = torch.nn.Embedding(num_embeddings=hp.LABEL_DIMENSIONS, embedding_dim=hp.EMBED_DIMENSIONS)
    ...
    def forward(self, x, labels=None):
        ...
        if labels is not None:
            ...
            # Convert the embedded labels into the same size/shape as the input image
            embedded_labels = embedded_labels.view(x.shape[0], hp.EMBED_DIMENSIONS, 1, 1)
            embedded_labels = embedded_labels.expand(-1, hp.EMBED_DIMENSIONS, x.shape[2], x.shape[3])

            # Embed the labels into the input image
            x = self.embedding_layer(torch.cat([x, embedded_labels], dim=1))
        ...
        real_fake_output = self.sigmoid(self.real_fake_layer(x))
