...
class MappingNetwork(nn.Module):
    ...
    def __init__(self):
        ...
        for _ in range(hp.MAPPING_LAYERS - 1):
...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        self.final_output = Conv2dLayer(hp.GEN_FEATURE_SIZE // 64, hp.NUM_CHANNELS, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, z1, z2=None, mixing_ratio=hp.MIXING_RATIO, labels=None):
        """
        Completes a forward pass of the generator, fully converting the 4x4 constant feature map into a full 
        resolution 256x256 greyscale image resembling the ADNI dataset images. If labels for the AD and CN classes are passed
        then their label information will be embedded into the style vector.
        ...
        # Convert to an image for output
...
class Discriminator(nn.Module):
    ...
    REF: This class was also inspired by code geenrated by ChatGPT-o1-preview via the following prompt:
    ...
    def __init__(self):
        ...
        # The amount of features present after flattening the features map and embedding labels
        self.feature_size = (hp.DISC_FEATURE_SIZE // 2) * 4 * 4
    ...
    def forward(self, x, labels=None):
        ...
        for either the AD or CN classes, then the labels will be embedded into the input image.
        ...
        real_fake_logits = self.real_fake_layer(x)
        ...
        if labels is not None:
            ...
            projection = torch.sum(x * embedded_labels, dim=1, keepdim=True)
        ...
        real_fake_output = real_fake_logits

        # Output class prediction (logits)
        class_ouput = self.class_layer(x)

        return real_fake_output, class_ouput, x
...
def l2_regularisation(gen):
    ...
    """
    # Sum the squared norms of the parameters
    l2_reg = torch.tensor(0.0, device=next(gen.parameters()).device)
    for param in gen.parameters():
        l2_reg += torch.norm(param, 2) ** 2
    
    return l2_reg * hp.L2_REG_TERM


def exponential_decay_with_warmup(optimizer, warmup_steps, decay_rate, total_steps):
    def lr_lambda(current_step):
        if current_step < warmup_steps:
            return float(current_step) / float(max(1, warmup_steps))  # Warm-up phase
        # Decay learning rate after warm-up, ensuring it decays across total_steps
        remaining_steps = total_steps - warmup_steps
        decay_steps = current_step - warmup_steps
        decay_factor = decay_rate ** (decay_steps / remaining_steps)
        return decay_factor
