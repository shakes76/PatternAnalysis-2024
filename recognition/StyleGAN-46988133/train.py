...
gen_opt = optim.Adam([{"params": [param for name, param in gen.named_parameters() if "mapping" not in name]},
                        {"params": gen.mapping.parameters(), "lr": 1e-5}], lr=hp.GEN_LEARNING_RATE, betas=(0.0, 0.99))
...
for current_depth in range(hp.START_DEPTH, hp.MAX_DEPTH + 1):
    ...
    gen_scheduler = exponential_decay_with_warmup(gen_opt, warmup_steps=total_steps_per_resolution*0.05, decay_rate=0.98, total_steps=total_steps_per_resolution)
    disc_scheduler = exponential_decay_with_warmup(disc_opt, warmup_steps=total_steps_per_resolution*0.1, decay_rate=0.96, total_steps=total_steps_per_resolution)
    ...
    for epoch in range(hp.EPOCHS_PER_RESOLUTION[current_depth]):
        ...
        for i, (real_images, class_labels) in enumerate(data_iter):
            ...
            # tot_loss_disc = (torch.mean(fake_pred) - torch.mean(real_pred)) + grad_penalty + (0.001 * torch.mean(real_pred ** 2))
            tot_loss_disc = (torch.mean(fake_pred) - torch.mean(real_pred)) + (0.001 * torch.mean(real_pred ** 2)) + grad_penalty + real_class_loss_disc + fake_class_loss_disc
            ...
            tot_loss_gen = -torch.mean(fake_pred) + loss_class_gen + l2_regularisation(gen)
