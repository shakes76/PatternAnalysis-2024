...
by: Karan Vijay Shankar (Student No. 47101425)
...
This architecture is comprised of 2D convolutional layers (with 3x3 kernel size and ReLU activation), followed by a 2x2 max pooling layer to downsample the input; extract features and patterns from the input image, while max pooling layers reduce spatial dimensions, allowing the network to focus on higher-level features. After reaching the latent layer, the output is passed through the *decoder* block; consists of two similar 2D convolutional layers, followed by a 2D transpose convolution layer (ConvTranspose2D) to upsample the feature map. *Concatenation* is used to combine upsampled features with their corresponding encoder features, enabling the retention of spatial details from previous layers. This process helps the network recover fine-grained information while ensuring gradients flow smoothly through the network during backpropagation, facilitating better learning and improved segmentation accuracy.

## How It Works

The code first takes the data found in the HIPMRI folders (specifically the training data), processing them (`utils.py`) into a dataset (`dataset.py`) to be used by the UNet model (`modules.py`). The UNet model is then trained on this dataset (`train.py`), with the goal of minimizing **Dice Loss**. The Dice coefficient is calculated to assess overlap between the predicted segmentations and ground truth labels. The model is then trained, ensuring the model is able to maintain/reach the Dice similarity coefficient of 0.75. After training, the trained model is saved; can be validated through running the saved model on the validation dataset; checking the images produced (`predict.py`).
...
Alternatively, `train.py` and `predict.py` can be ran individually to either train/validate the model (`train.py` must be run first if a model has not been generated yet for `predict.py` to function)
...
```
As can be seen here, with fine-tuning of hyperparameters, the UNet model was able to acheive above 0.75 similarity coefficient on the dataset
...
![Example of UNet model output](images/model-example-3e_t.png)
