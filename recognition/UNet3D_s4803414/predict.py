...
with torch.no_grad():
    for batch_idx in range(0, len(image_filenames), BATCH_SIZE):
        ...
        for i in range(images_tensor.size(0)):
            image = images_tensor[i].cpu().numpy().squeeze()  # Convert to numpy and squeeze
            mask = masks_tensor[i].cpu().numpy().squeeze()  # Convert to numpy and squeeze
            pred = predictions[i].cpu().numpy()  # Get prediction without squeezing yet

            # Loop through all slices
            for slice_idx in range(image.shape[2]):
                image_slice = image[:, :, slice_idx]
                mask_slice = mask[:, :, slice_idx]
                pred_slice = pred[:, :, slice_idx]  # Get prediction for the current slice
        ...
        # Save visualization function (unchanged)
        def save_visualization(image_slice, mask_slice, pred_slice, slice_idx, save_path):
            # Create the figure for each slice
            fig, axes = plt.subplots(1, 3, figsize=(15, 5))
            ...
            # Plot predicted slice (make sure it's correctly displayed)
            axes[2].imshow(pred_slice, cmap='viridis')
            axes[2].set_title(f'Prediction Slice {slice_idx}')
            axes[2].axis('off')
