...
def load_data_2D(image_names, norm_image=False, categorical=False, is_mask=False, dtype=np.float32, get_affines=False, early_stop=False, target_size=(256, 256)):
    '''
    Load medical image data from names, cases list provided into a list for each.

    This function resizes images to a fixed size to ensure consistency.

    is_mask: bool (indicates if the data is a mask)
    ...
    for i, inName in enumerate(image_names):
        ...
        if is_mask:
            ...
            inImage = resize(inImage, target_size, order=0, preserve_range=True, anti_aliasing=False)
            # Binarize the mask
            inImage = (inImage > 0.5).astype(dtype)
        else:
            # Resize image with bilinear interpolation
            inImage = resize(inImage, target_size, order=1, preserve_range=True, anti_aliasing=True)
            if norm_image:
