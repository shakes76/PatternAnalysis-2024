...
### Results
The model's training and validation loss curves (in the graphs folder) indicate that it effectively learned throughout the 10 epochs, as both losses show a decreasing trend, suggesting convergence without significant overfitting. After tuning batch size, patch size, number of attention heads, weight decay and learning rate, the final test accuracy achieved is 59.57%. Better utilising larger patch and batch sizes, perhaps increasing the number of self attention heads (although I beleive 8 is a good compromise) as well as decreasing learning rate from 0.001 have been tested at somewhat small epochs of 5 to 10, each demonstrating slight improvements in accuracy from 50%. The results here are based off of 10 epochs of the hyperparameters -- tuned to compromise quality for accuracy e.g. patch size of 16, learning rate of 0.001. Increasing the complexity of the code can cause extremely long run times and as such, a great improvement to this solution would be to complete a run of 30-100 epochs with the hyperparameters tuned for increased detail for pattern recognition. 

From the classification report, the precision, recall, and F1-score values reveal a slight imbalance in the model’s performance across the two classes: "NC" (Normal Control) and "AD" (Alzheimer's Disease). Specifically, the model performs better on "AD" with a recall of 0.67 compared to 0.52 for "NC", showing that it identifies "AD" cases more effectively.

The confusion matrix further supports these findings, showing a larger number of true positives for "AD" and more false negatives for "NC". Overall, while the model demonstrates some capability to distinguish between "NC" and "AD", improvements are needed to enhance its accuracy and balance across classes.
