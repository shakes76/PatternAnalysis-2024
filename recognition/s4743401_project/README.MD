...
### Results
The model's training and validation loss curves (in the graphs folder) indicate that it effectively learned throughout the 10 epochs, as both losses show a decreasing trend, suggesting convergence without significant overfitting. After tuning batch size, patch size, number of attention heads, weight decay and learning rate, the final test accuracy achieved is 59.57%. Better utilising larger patch and batch sizes, perhaps increasing the number of self attention heads (although I beleive 8 is a good compromise) as well as decreasing learning rate from 0.001 have been tested in isolation, at somewhat small epochs. Increasing the complexity of the code can cause extremely long run times and as such, a great improvement to this solution would be to complete a run of 30-100 epochs with the hyperparameters tuned for increased detail for pattern recognition. 
