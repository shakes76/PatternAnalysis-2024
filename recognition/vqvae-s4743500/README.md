...
### Vector Quantizer  
The Vector Quantizer module is one of the most important components of the VQ-VAE architecture. In my implementation, I used nn.Embedding to initialise the codebook with `num_embeddings` unique vectors, where each of them have a size of `embedding_dim`  
...
## 3. VQ-VAE Training  
Before beginning the training process, I saved a fixed batch of original training images:  
  
DISPLAY ORIGINAL IMAGES HERE  
  
This was used as reference when comparing it against the reconstructed images generated by the VQ-VAE model
